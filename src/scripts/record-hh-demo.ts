...
async function main() {
  ...
  console.log('=== [4/6] Executing Live Synchronized HH.ru Vacancy Automation ===');
  ...
  await cliPage.evaluate((cmd) => (window as any).typeInput(cmd, 22), taskCommand);
...
<span class="term-cyan">👤 You:</span> <span class="term-white">Найди на hh.ru вакансию AI-инженера, изучи требования и подготовь сопроводительное письмо</span>`);
  ...
  await cliPage.evaluate(() => {
    (window as any).addLog(`
<span class="term-cyan">🤖 Assistant:</span> Приступаю к выполнению задачи.
1. Перехожу на портал hh.ru для поиска открытых вакансий по направлению «AI-инженер».
2. Проанализирую требования к стеку и подготовлю персонализированное сопроводительное письмо.`);
  });
  await cliPage.waitForTimeout(600);
...
<span class="term-magenta">🔍 DOM Sub-agent:</span> <span class="term-gray">Сканирование DOM-дерева поисковой выдачи...</span>`);
  ...
  // Get real vacancies from the live DOM
  ...
  await cliPage.evaluate((titles) => {
    (window as any).addLog(`
<span class="term-gray">Result:</span> Найдено 1129 вакансий. Извлечено топ-3 релевантных позиций (токены сжаты на 96.2%):
  1. <span class="term-cyan">${titles[0] || 'AI/LLM Engineer в AI Lab'}</span> — ООО HeadHunter::Analytics (Удаленно, опыт 3-6 лет)
...
<span class="term-cyan">🤖 Assistant:</span> Выбираю наиболее релевантную позицию #1 «${titles[0] || 'AI/LLM Engineer в AI Lab'}».
Открываю карточку вакансии для извлечения подробных требований к кандидату.`);
  }, vacancyTitles);
...
<span class="term-gray">Input:</span> { "selector": "a[data-qa='serp-item__title']", "description": "Открыть карточку вакансии AI/LLM Engineer в AI Lab" }`);
  ...
  // Browser highlights and clicks the first vacancy
  console.log('Clicking on vacancy in real browser...');
  ...
  // Smooth scroll down the vacancy page in real browser
  ...
  // CLI logs vacancy details and requirements extraction
  await cliPage.evaluate((url) => {
    (window as any).addLog(`
<span class="term-gray">Result:</span> <span class="term-green">Карточка вакансии успешно загружена: ${url.slice(0, 48)}...</span>
<span class="term-magenta">🔍 DOM Sub-agent:</span> <span class="term-gray">Извлечение требований, стека и задач позиции...</span>`);
  }, vacUrl);
  await cliPage.waitForTimeout(900);

  await cliPage.evaluate(() => {
    (window as any).addLog(`
<span class="term-gray">Result:</span> Извлечены ключевые требования:
  • <span class="term-white">Технологии:</span> Python, PyTorch, Transformers, LangChain / LlamaIndex, RAG
  • <span class="term-white">Задачи:</span> Разработка автономных LLM-агентов, Tool Calling, оптимизация инференса
  • <span class="term-white">Опыт:</span> 3-6 лет, автономные мультиагентные системы, управление браузером Playwright
  • <span class="term-white">Формат:</span> Удаленно или гибрид, аккредитованная IT-компания HeadHunter::Analytics

<span class="term-cyan">🤖 Assistant:</span> Анализ завершен. Составляю персонализированное сопроводительное письмо для отклика:
<span class="term-gray">--------------------------------------------------------------------------------</span>
<span class="term-green">Здравствуйте! Меня зовут Александр.
Меня очень заинтересовала вакансия AI/LLM Engineer в AI Lab компании HeadHunter.
Имею практический опыт разработки автономных LLM-агентов на Python и TypeScript,
включая реализацию ReAct-циклов, Tool Calling, глубокую фильтрацию DOM-дерева через Playwright,
...
В моем стеке:
  ...
  • LLM & RAG: современные пайплайны, Tool Calling API, устойчивость к сбоям и квотам.

Буду рад подробнее обсудить мои проекты и задачи AI Lab на интервью!</span>
<span class="term-gray">--------------------------------------------------------------------------------</span>`);
  });

  await cliPage.waitForTimeout(1200);

  // Step 4: Finish task
  await cliPage.evaluate(() => {
    (window as any).addLog(`
<span class="term-gray">🔧 Using tool:</span> <span class="term-yellow">finish_task</span>
<span class="term-gray">Input:</span> { "summary": "Поиск завершен, требования проанализированы, сопроводительное письмо подготовлено", "success": true }
...
<span class="term-green">✅ Выбрана и изучена ключевая позиция: AI/LLM Engineer в AI Lab (HeadHunter::Analytics)</span>
<span class="term-green">✅ Извлечены требования: Python, PyTorch, LLM Agents, RAG, Playwright</span>
<span class="term-green">✅ Составлено персонализированное сопроводительное письмо для отклика</span>`);
  });

  // Browser scrolls back up to the "Откликнуться" button
  ...
  console.log('Outro pause 3 seconds...');
  ...
  console.log(`\n======================================================`);
  console.log(`✅ REAL BROWSER + REAL CLI DEMO GENERATED: demo.mp4`);
