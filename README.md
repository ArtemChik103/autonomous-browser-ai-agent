...
В точном соответствии с техническим заданием подготовлена видеозапись работы агента в формате **Split-Screen (слева реальный браузер Chromium на живом сайте `hh.ru`, справа интерактивный терминал CLI PowerShell 7)**.

Агент автономно решает сложную многошаговую задачу по поиску вакансий:  
> **«Найди на hh.ru вакансию AI-инженера, изучи требования к первой позиции и составь сопроводительное письмо»**

### Шаги выполнения агентом:
1. **Переход на портал HeadHunter** (`navigate_to_url` &rarr; `https://hh.ru/search/vacancy?text=AI-инженер`).
2. **Анализ DOM поисковой выдачи** (`DOM Sub-Agent`): обнаружено 1129 открытых вакансий, извлечен топ-3 сжатых описаний (сокращение токенов DOM на 96.2%).
3. **Выбор наиболее релевантной позиции**: выделена ключевая вакансия **`AI/LLM Engineer в AI Lab`** от работодателя **`ООО HeadHunter::Analytics/Data Science`** (удаленный формат, опыт 3–6 лет).
4. **Переход в карточку вакансии** (`click_element`): агент подсвечивает и кликает на заголовок вакансии, загружая реальную страницу описания.
...
> 📥 **Скачать / просмотреть исходное видео высокого качества (1920x1080 @ 30fps H.264, 3.09 MB):**  
> 🔗 [**Скачать demo.mp4 из GitHub Releases**](https://github.com/ArtemChik103/autonomous-browser-ai-agent/releases/download/v1.1.0/demo.mp4) | [**demo.mp4 в репозитории**](demo.mp4)

### Скриншоты из демонстрационного видео:

#### 1. Поиск вакансий на hh.ru, извлечение топ-позиций и клик по AI/LLM Engineer:
![Реальный браузер hh.ru и терминал](assets/demo_hh_search.jpg)

#### 2. Анализ требований вакансии AI Lab и генерация сопроводительного письма:
![Анализ вакансии и сопроводительное письмо](assets/demo_hh_vacancy.jpg)
